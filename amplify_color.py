...
class VidMag:
    ...
    def magnify_color(
        self, data_buffer, fps, low=0.4, high=2, levels=3, amplification=30
    ):
        gau_video = self.gaussian_video(data_buffer, levels=levels)
        filtered_tensor = self.temporal_ideal_filter(gau_video, low, high, fps)
        amplified_video = self.amplify_video(
            filtered_tensor, amplification=amplification
        )
        final_video = self.reconstract_video(
            amplified_video, data_buffer, levels=levels
        )
        # print("c")
        return final_video
    ...
    def magnify_motion(
        self, video_tensor, fps, low=0.4, high=1.5, levels=3, amplification=30
    ):
    ...
    def mainLoop(self):
        ...
        cv2.putText(
            frame,
            "FPS " + str(float("{:.2f}".format(self.fps))),
            (20, 420),
