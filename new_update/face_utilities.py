...
class Face_utilities:
    ...
    def age_gender_detection(self, face):
        ...
        blob = cv2.dnn.blobFromImage(
            face, 1, (227, 227), self.MODEL_MEAN_VALUES, swapRB=False
        )
        # Predict gender
    ...
    def get_landmarks(self, frame, type):
        ...
        if self.predictor is None:
            ...
            self.predictor = dlib.shape_predictor(
                "../shape_predictor_" + type + "_face_landmarks.dat"
            )
    ...
    def ROI_extraction(self, face, shape):
        ...
        if len(shape) == 68:
            ROI1 = face[
                shape[29][1] : shape[33][1], shape[54][0] : shape[12][0]  # right cheek
            ]

            ROI2 = face[
                shape[29][1] : shape[33][1], shape[4][0] : shape[48][0]  # left cheek
            ]
        else:
            ROI1 = face[
                int((shape[4][1] + shape[2][1]) / 2) : shape[4][1],  # right cheek
                shape[2][0] : shape[3][0],
            ]

            ROI2 = face[
                int((shape[4][1] + shape[2][1]) / 2) : shape[4][1],  # left cheek
                shape[1][0] : shape[0][0],
            ]
